...
$importAction = function () {
        ...
        if (is_array($decodedTranscript) && isset($decodedTranscript['lines']) && is_array($decodedTranscript['lines'])) {
          foreach ($decodedTranscript['lines'] as $line) {
            ...
            $timestamp = $normalizeJsonTimestamp((string) ($line['startTime'] ?? $line['startTie'] ?? ''));
...
$episodeOverviewAction = function () {
        ...
        foreach ($allEpisodes as $episode) {
          ...
          if (preg_match('/(?:^|[\/_\-\s])s0*(\d+)(?:$|[\/_\-\s])/i', $seasonHaystack, $seasonMatch) === 1) {
          ...
          $blockField = (string) $episode->content()->get('blocks')->value();
        ...
        usort($episodes, static function (array $a, array $b): int {
          $numberA = $a['episodeNumber'] ?? null;
          $numberB = $b['episodeNumber'] ?? null;
